...
| 12 ✓ | `fix(blast)`: damage by `W·η` through all four verdict paths. Flown with 50 Mt over the 5"/54 mount: at 70 km the parent loaded two parts and called a near miss, this loads nothing; at 33 km both break the part. **`dev` is not merged to `main` until step 14 is flown** |
| 13 ✓ | `fix(blast)`: the front, arrivals, the drawn front and the ring on the burst's air; every `ShockRadius` consumer listed and moved together, including `WeaponSystem.cs:3838`'s log. They all ask one `BlastFront`, doubled only by ground coupling. Flown: 0.3 kt at 800 m now reaches the mount under it at 2.09 s, against 2.06 with the energy doubled; 20 kt at 900 m still bends the view as a dome and stands its cap over a short column. The dust ring's speed in the shader stays the sea-level constant |
| 14 ✓ | `fix(sound)`: hear and feel a high burst -- the `IsThin` split and the live-front bang. `BurstSound` follows each burst's `BlastFront` to the camera and strikes the shake as it plays the bang, gated at 50 Pa, with no 45 s cap. Flown: 20 kt at 6 km, 16.5 s and 5.1 kPa as before; 50 Mt at 37 km overhead, heard and felt at 59.3 s and 11.4 kPa, where it was silent; 3.8 Mt at 77 km, 233 s and 150 Pa, faintly -- KSA's air at 77 km is four times the real, and unheard from about 88 km. The front law's 59 s agrees with a Mach number integrated from the pressure rule (66 s) and Yucca's 197 s with 195: the ground observer's figure of height over sound speed was too rough, since the front runs strong through the thin air |
| 15 ✓ | `feat(flash)`: pulse, fraction, glow and fireball size by air; an airless burst's flash keyed on air 0, which only then has a size of its own. `ThermalAltitude` carries G&D's pulse law, the fraction's knots, the glow (Orange 17.4 s, Teak 2.4 s) and the single pulse; the thin-air ball stops at 8x, Teak's. Flown: Teak is 26.6 km across, glows 3.8 s and is white-hot 1.9 s; 1 Mt at 40 km glows 13.6 s; a 20 kt burst on the Moon whites the view out and is gone by 0.5 s, where it was a sea-level ball; 20 kt on the ground unchanged. The regime pin moves where the ball's cap binds |
| 16 | `refactor` then `feat(clouds)`: `CloudFlags`, then the dry ball. **`CloudFlags` ✓**: packed in Sim, three bits of dryness, the front held at 262,143 m; the decode's `+ 0.5` is gone, since at the top of the range it rounded 2^24 - 1 up a metre |
