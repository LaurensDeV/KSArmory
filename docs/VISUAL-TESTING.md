...
- **The mod** polls `<user dir>/Logs/bridge/in/` ten times a second, runs each command file it
  ...
  *pause*, *resume*, *speed*, *step* (run so many simulated seconds, then pause), *camera* (a pose
  ...
  warhead would judge it, the flown one as its platform), *dents* (what the engine holds on the flown
  craft, each with its push against the line from the last damaging burst, or *clear* them), *capture* (3–5), *set* and *get* (a `Config` field),
