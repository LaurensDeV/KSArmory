...
| Heat and the shock front visibly bend the view round the ball | **The shock front is built; heat shimmer is not.** A dome racing out on `MushroomCloud.ShockRadius`, drawn as a lens across its shell: the scene is copied and written back at an offset where a ray grazes it, so it bends whatever is behind and hides nothing (`Shaders/KSArmoryShock.comp`). Real air bends light by about 0.03° there — a pixel, which is why the real front is only seen on high-speed film — so it is drawn at fifteen times that, fading with the overpressure. Flown: a 20 kt dome from 12 km reads clearly at 6 s, and at the physical strength same-instant captures differed by nothing visible. **Every front is drawn**, a copy and a bend each, so crossing fronts bend what the one before left and their bends add: three 20 kt fronts 3 km apart cost 0.14–0.19 ms at 1440×900 against the pass short-circuited, and one 0.07. Nothing once they have faded |
...
- **Heat shimmer** needs the scene read at an offset while it is being written, which is a copy of the
  ...
  out. The shock ring now pays for that copy (`KSArmoryShock.comp`, full-screen, 0.07 ms), so
