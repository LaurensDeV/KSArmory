...
| The flash throws hard shadows across everything in sight | **Checked on 2026-09-23 and not built.** The ball's light visibly reaches nothing at 350–900 m at night: trees facing it stay silhouettes, flown with the camera turned away from the burst (the bridge's `turn_deg`) and the whiteout switched off for the look. The terrain takes point lights through KSA's forward list, which has no shadows at all, so a shadow could only fall from a craft or a part. Casting shadows at 120 ball radii was built and taken out: unverified, and a shadow-casting light 6 km across renders a cube shadow map every frame. A light 100 times brighter drove the view black, which reads as the eye's exposure adapting to the ball rather than as the light arriving — the next thing to settle |
