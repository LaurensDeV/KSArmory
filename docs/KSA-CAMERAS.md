...
Every citation was re-checked against the build named below, and the ones that had moved were
moved with it. A citation is still a pointer rather than a proof: check before relying on one.
...
> `docs/KSA-MODDING-NOTES.md` viewport section has the new shape; **everything here about
> controllers, frame order, `Camera` and the reference frames still holds**, because none of that
> was touched. Re-deriving the 393 citations is its own piece of work and has not been done.
...
`Program.OnFrame` (`KSA/KSA/Program.cs:2066`) in order:

| # | Step | Ref |
|---|---|---|
| 1 | `PrepareFrame` — orbit/vehicle solvers applied, `Glfw.PollEvents()` | `Program.cs:2000-2064` |
| 2 | `OnFrameViewports(dtPlayer)` — **every controller writes its camera here** | `Program.cs:2091`, `2414-2468` |
| 3 | `OnDrawUiFrame` / `OnDrawUiViewports` (StarMap Gui hooks wrap these) | `Program.cs:2095-2096` |
| 4 | `OnFrameController` (cursor mode), `Cursor.UpdateInputRay(GetCamera())` | `Program.cs:2143-2146` |
| 5 | `OnPreRender` → `Render` → `PostRender` | `Program.cs:2151-2159` |
...
`OnFrameViewports` (`Program.cs:2414-2468`) calls `viewport.OnFrame((float)dtPlayer)` for **every**
viewport, visible or not (the call is at `:2432` and the `Visible` test at `:2434`). The dt is player
time, cast through `float`, and keeps ticking while the simulation is paused.

`Viewport.OnFrame` (`KSA/KSA/Viewport.cs:145-150`) is exactly three calls:
`GetActiveController().OnFrame(this, dt)` → `GetCamera().OnFrame(dt)` → `ViewportAudio(dt)`.

`Camera.OnFrame` (`Camera.cs:482-492`) calls `ClampCamera()` **first**, then builds the view/VP
...
`Program.ViewportCount = 6` (`Program.cs:249`), built at `Program.cs:900-931`:

| Index | Role | State at boot |
|---|---|---|
| 0 | `MainViewport` | `Visible = true`, `LightMode = Clustered`, no own render target (shares `_offscreenTarget`) |
| 1 | `ThumbnailViewport` | `IsOffscreen = true`, `ShouldRenderGizmos = false`, `LightMode = None`, `ThumbnailRenderer.SIZE` square |
| 2, 3 | spare | `Visible = false`, own 500×500 render targets |
| 4, 5 | crew portraits (`_crewPortraitViewportStart = 4`) | `IsOffscreen = true`, `Visible = true`, `Mode = Fixed`, `ShouldRenderStars = false`, `AllowResize = false`, 128 square, 42° |

So a mod still has **at most two spare viewports** for a picture-in-picture camera — the other four
are spoken for — and `DockingPort` competes for the same two (`KSA/KSA/DockingPort.cs:181-192` scans
for the first viewport that is neither `Visible` nor `IsOffscreen`). `ViewportCount` is a public
static field but changing it after boot creates nothing.

Public and writable from a mod: `Viewport.Mode`, `.Visible`, `.IsOffscreen`, `.ShouldRenderGizmos`,
`.ShouldRenderStars`, `.LightMode`, `.NewSize`, `.AllowResize`, `.BaseCamera`, `.MapCamera`, and all
five controllers (`Viewport.cs:14-56`). `Program.Viewports` is a `public static readonly
List<Viewport>` (`Program.cs:251`).

- `Viewport.GetCamera()` returns `MapCamera` **only** in `Map` mode, `BaseCamera` otherwise
  (`Viewport.cs:332-339`). The four non-Map controllers are all constructed against `BaseCamera`;
  `MapController` alone gets `MapCamera` (`Viewport.cs:117-121`). So `FixedController.Camera == BaseCamera`.
- `Viewport.GetActiveController()` is a switch that **throws `ArgumentOutOfRangeException` on the
  default arm** (`Viewport.cs:319-330`). Writing a bogus `CameraMode` into the public `Mode` field
...
- Non-main viewports get an ImGui window of their own automatically, named `ViewportName`
  (`Viewport.DrawImGui`, `Viewport.cs:239-298`, called from `Program.cs:2921-2933`). The window's
  close button writes `Visible` by ref; nothing resets the camera mode when it closes — `DockingPort`
  detects the close by polling `Viewport.Visible` (`DockingPort.cs:172-174`).
- In `Fixed` mode the viewport's menu bar and hover rectangle are suppressed (`Viewport.cs:290`).

#### Is a mode change immediate?

`Viewport.SetCameraMode` (`Viewport.cs:341-351`) is **synchronous and immediate** for the *field*:
...
(`MapController.cs:573-577` restores the last map pose; `IVAController.cs:187-194` sets `LocalRotation`),
the other three do not.

`Program.SetCameraMode(mode)` (`Program.cs:2875-2877`) targets `HoveredViewport`, not the main one.
`Viewport.NextCameraMode()` cycles Orbit→Free→IVA→Orbit only; from `Map` or `Fixed` it returns false
and does nothing (`Viewport.cs:353-369`). **A viewport in `Fixed` mode can only be taken out of it by
...
bookkeeping (`MapController.cs:556`, `:594`) and `IVAController`'s seat/`LastFollowing` setup.
...
(`Camera.cs:79`) is the only thing `Camera.OnFrame` feeds into the view matrix (`Camera.cs:485-490`).
...
- `TidalLocking` (`Camera.cs:160`, set at `:604`) is stored, saved (`:810`) and shown in a menu
  (`Program.cs:3461`) but **is not read by any code that affects the camera pose** — grep across the
...
`SetFollow(target, tidalLocking, changeControl = true, alert = true)` (`Camera.cs:597-613`):
- immediately jumps the camera to `target.GetPositionEcl() + (float)target.MeanRadius * 2.5 * forward`.
  Note the `(double)(float)` narrowing of `MeanRadius` at `:601`.
...
- `Unfollow(changeControl = true)` preserves `PositionEcl` and clears `_tidalLocking` (`Camera.cs:615-626`).

FOV: `SetFieldOfView` / `SetOrthographic` / `SetOrthoHalfHeight` (`Camera.cs:412-438`) and
`ChangeFieldOfView` (`:450-457`), the last clamped 15–120° (`Camera.cs:37-39`, `:454`). Applied once
at viewport creation from `GameSettings` (`Program.cs:634`, `:638`, `GameSettings.cs:2415-2418`) —
**not overwritten per frame**, so a mod's FOV sticks. `Camera.OnKey` (`Camera.cs:840-858`) is
dispatched *before* the active controller (`Program.cs:1667`), so the player's FOV keys work in every
mode including `Fixed`.

`Camera.SerializeSave` / `DeserializeSave` (`Camera.cs:801-833`) store `Following.Id` and resolve it
...
(`KSA/KSA/OrbitView.cs:5-13`) — `ReferenceFrame`, `Azimuth`, `Elevation`, `DistancePower`,
...
frame/origin shift. `Program.GetOrbitController()` (`Program.cs:614`) returns the `FrameViewport`'s.
...
- `GetCarousel2Cce` (`:317-333`, used by `CameraReferenceFrame.Parent`) dereferences
  `celestial.Orbit.Parent` — NRE if `Orbit` is null. It guards the degenerate cross-product case by
  returning `Identity` (`:325-328`), as `FixedController`'s copy now does too
  (`FixedController.cs:110-113`).
...
- `GetFrame2Ecl` (`:577-651`) **throws by design** for invalid combinations:
...
mode (`Viewport.cs:119`, `:332-339`).

**`OnFrame` (`:125-290`)**: bails to `Program.SetCameraMode(CameraMode.Free)` if nothing is followed
(`:127-131`) — note that targets the **hovered** viewport, not this one. Otherwise it applies drag/keys
to its **own** `OrbitView` field (`:44`, not the followed object's) and `Scope`, then writes every
frame:

```
Camera.PositionEcl   = Lerp(AnimationStartPosition, AnimationEndPosition, AnimationProgress)   (:282)
Camera.LocalRotation = LookAtRotation(dir, up)                                                 (:283)
```

**Intended external interface:** `Scope` (metres, floored at `Following.MeanRadius` at `:270`),
`PositionOffsetFollowing` (`:36`), the controller's own `OrbitView` (`:44`), `Inverted`,
`PreviouslyControlledVehicle`, `AnimateFocusChange` + `AnimationStartPosition`, `VehicleControlToggle` /
`ToggleVehicleControl()` (`:109-123`). `SetDefaults()` (`:75-107`) computes a sane `Scope` from the
target's radius and SOI. `Program.GetMapController()` at `Program.cs:624`.

**Traps:**
- `OnSwitchOn` sets `Camera.NoRotation = true` (`:556`) and `OnSwitchOff` sets it false (`:594`).
...
- `OnSwitchOff` unconditionally sets `Program.ControlledVehicle = PreviouslyControlledVehicle` and
  `IsControlledVehicleActive = true` (`:590-591`) — leaving map mode hands control back whether the
  mod wanted it or not.
- `MeanRadius` and `GetPositionEcl` on `Camera.Following` are dereferenced unguarded at `:270`, `:273`.
...
2. Position, **every frame** (`:40-41`):
   ...
   ```
   `PositionVehicleAsmbOffset` is `offset.Transform(MatrixAsmb2VehicleAsmb)` (`KSA/KSA/Part.cs:1109-1112`);
   `PosAsmbToBody` is `posAsmb - CenterOfMassAsmb` (`Vehicle.cs:1218-1221`). So the seat point is taken
...
reach it as `viewport.IVAController` (`Viewport.cs:52`).
...
(`IVASeat.cs:9-27`, built from the template at `:122-130`). **A mod can declare an `IVASeat` on its
own part XML** and IVA mode will find it — `vehicle.Parts.Modules.Get<IVASeat>()`
(`IVAController.cs:128`, `:164`).
...
#### 4.5 Fixed — `FixedController` (`KSA/KSA/FixedController.cs`, 144 lines)

**For:** a camera pinned to a point on a followed object, aimed along a fixed direction. The engine's
only user is the docking-port camera (`KSA/KSA/DockingPort.cs:181-192`), and it no longer goes through
the two public fields — see `DockingConnector` below.

**`OnFrame` (`:22-48`)** — the whole thing:

```csharp
IFollowable following = Camera.Following;
if (following != null) {                                              // :24-25
    if (DockingConnector != null && following is Vehicle vehicle) {    // :27  <-- ignores both fields
        …docking-camera pose from the connector…                      // :29-33
        return;                                                       // :34
    }
    OrbitView   orbitView = following.OrbitView;                       // :36
    doubleQuat  frame2Ecl = GetFrame2Ecl(following, orbitView.ReferenceFrame);   // :37
    double3     up        = double3.UnitZ.Transform(frame2Ecl);        // :38
    double3     right     = double3.Cross(CameraRotation, up).Normalized();      // :40  <-- throws
    double3     upEcl     = double3.Cross(right, CameraRotation).Normalized();   // :42  <-- throws
    Camera.PositionEcl    = following.GetPositionEcl() + CameraOffset; // :44-45
    Camera.LocalRotation  = Camera.LookAtRotation(CameraRotation, upEcl);        // :43, :46
}
```
...
`LookAtRotation` normalises the forward internally (`Camera.cs:200`), so `CameraRotation` need not be
unit length — but it must not be zero.

**Roll** is not settable. It is fixed by `frame2Ecl`'s +Z axis (`:38`), which comes from
`Following.OrbitView.ReferenceFrame` — a field the mod does not own and the player can change.

**The engine never writes `CameraOffset` or `CameraRotation`, and no longer reads them for its own
camera either.** `DockingPort` sets a third public field, `DockingConnector` (`:13`), and
`FixedController` then derives the whole pose from that connector every frame (`:27-35`) — so the stock
docking camera tracks the vehicle's rotation and the two `double3` fields are the *mod* path alone. To
stay attached to a rotating body a mod must still rewrite both **every frame**, from a
...
2. **Same crash if `CameraRotation` is parallel to `frame2Ecl`'s +Z axis** — the cross at `:40` is again
...
3. `:42` and `Camera.LookAtRotation` (`Camera.cs:200-202`) call `Normalize` again — same failure mode.
4. **`GetFrame2Ecl` (`:50-100`) unwraps nullables without checking**, unlike `OrbitController`'s copy:
   - `:58` `vehicle.GetEnu2Cce().Value` — `ComputeEnu2Cce` returns null when the vehicle's CCE position
     is near zero or exactly on the parent's rotation axis (`Vehicle.cs:2982`, `:2985-2996`).
   - `:62-63` `vehicle.GetLvlh2Cce().Value` — `ComputeLvlh2Cce` returns null when CCE velocity or
     position is near zero, or when they are parallel (`Vehicle.cs:2959`, `:2961-2970`).
     ...
     `Orbit` in low orbit, `Parent` in high orbit (`VehicleRegionEx.cs:39-67`, applied at
     `Vehicle.cs:2926-2928`) — so a mod does not control which branch is taken.
   - `:66` `Parent` → `GetCarousel2Cce` → `celestial.Orbit.Parent.GetCci2Cce()` (`:104`), NRE if `Orbit`
     is null.
5. **`GetFrame2Ecl` has no `WreckageMarker` branch** (`OrbitController.cs:275-309` does). When a
   ...
   for every viewport (`Universe.cs:1710-1720`, `Camera.FollowWreckage` at `Camera.cs:590-595`), whose
   `ReferenceFrame` is forced to `Surface` or `Orbit` (`WreckageMarker.cs:53`). `FixedController` falls
   into the `else` arm, whose only case is `VehicleEditingSpace`, and returns `Identity` (`:97`) — so
   the roll reference jumps to Ecl +Z the instant the target dies, which can turn trap 2 on without the
   mod doing anything.
6. `CameraOffset`, `CameraRotation` **and `DockingConnector` persist across mode switches** —
   `OnSwitchOn` (`:140-143`) only raises a `TimedAlert`, and `OnSwitchOff` is not overridden. A stale
   `DockingConnector` is the worse of the two: it takes the branch at `:27` and the mod's own fields are
   never read. `DockingPort.ResetDockingViewport` clears it (`DockingPort.cs:140`), so this needs the
   docking camera to have been abandoned rather than closed.
...
**Nothing in the engine attaches a camera to a character or a bone. A mod can still read a bone and
drive `Fixed` mode from it.**

- The only first-person mechanism in the engine is `IVAController` + `IVASeat`, and that is a
  **part module on a vehicle**, not a character attachment (`IVASeat.cs:7`, built from the part
  template at `:122-130`).
- `CharacterAvatar` (`KSA/KSA/CharacterAvatar.cs:12`) is a pure render description — core mesh, fur,
...
- Bone transforms exist, in two flavours that differ by which frame they land in.
  `AnimatedRenderable.GetBoneTransform(int)` and `GetBonePosition(int)`
  (`KSA/KSA/AnimatedRenderable.cs:236-262`) multiply by `Transform`, which is set to a
  **camera-relative (Ego) `float4x4`** during rendering (`KittenRenderable.cs:346`, from
  `KittenEva.UpdateRenderData` → `Vehicle.GetWorldMatrix(camera)`, `Vehicle.cs:3501-3512`) — float
  precision, relative to *last* frame's camera, and only valid after rendering, which is the wrong side
  of the frame from where a camera must be positioned. `GetBoneLocalPosition(int)` (`:246-253`) does
  not: it is the animated skeleton's own translation, in model space, with no camera in it.
- **A mod can reach the animated skeleton.** `KittenEva.Renderable` is a public getter
  (`KittenEva.cs:59`), and `KittenRenderable` exposes `TryFindBoneIndex(name, out index)` (`:114`) and
  `GetBoneAsmbOffset(index)` (`:109`), the latter carrying the bone's local position through
  `ModelToBodyMatrix()` (`:104-107`) into the **vehicle assembly frame**. That is a head position in the
  frame a `Fixed` camera offset wants, and it is what `IVASeat.GetSeatedBonePointAsmb` (`IVASeat.cs:62`)
  is built on. `KittenRenderable._characterAvatar` is still private with no accessor
  (`KittenRenderable.cs:12`), and `KittenEva.Character` is a `CharacterReference` (asset data) rather
  than the live avatar — neither is needed for a bone offset.
...
`LookTarget` (`:18`) and `MaxLookAtAngle = 30°` (`KittenRenderable.cs:218`) — it aims the *eyeballs* at
...
(`KittenEva.cs:319-332`) copies the **main camera's** basis into
`CharacterControlInputs.{CameraForwardCce, CameraRightCce, CameraUpCce}` every physics step (`:327-329`)
— that is the camera driving the kitten's movement frame, not the other way round.
`KittenEva.CharacterControlInputs` and `LocomotionState` are public getters (`:49-51`).

**What actually works for a kitten POV:**
- `KittenEva : Vehicle` (`KittenEva.cs:13`), so it is an `IFollowable` and every mode accepts it.
- `Fixed` mode following the kitten, with `CameraOffset` recomputed each frame the way
  ...
  `offsetBody.Transform(kitten.Body2Cce)` — and `CameraRotation` from `kitten.Body2Cce`. The offset can
  be a real head bone rather than a guessed height, through `Renderable.GetBoneAsmbOffset` above.
  `Vehicle.GetBodyFixed2Ecl()` is `Body2Cce` (`Vehicle.cs:1241-1244`); `Asmb2Cce`/`Body2Ego`/`Asmb2Ego`
  are all the same quaternion (`Vehicle.cs:469-503`).
...
`KittenEva.UpdateHighlight` (`:1097-1108`) is worth noting: kitten hover-picking is disabled unless
...
|---|---|---|
| `Astronomical` (abstract) | `Astronomical.cs:12`, `:369` | abstract |
| `Vehicle` | `Vehicle.cs:28`, `:891-894` | `_positionEcl`, cached |
| `Celestial` | `Celestial.cs:23`, `:388` | analytic orbit position |
...
`_positionEcl`, assigned in `Vehicle.UpdatePerFrameData` (`Vehicle.cs:2456-2461`) as
...
(`Camera.cs:590-595`). The engine swaps to it automatically when a followed vehicle is destroyed
(`Universe.cs:1710-1720`). Its `GetPositionEcl` (`:85-93`) recomputes from either a body-fixed
...
downcasts to `Astronomical` unguarded (representative call sites: `Program.cs:3147`, `:3413`, `:3502`,
`LocationMusicPlayer.cs:88-95`, `PhysicalAtmosphereReference.cs:61`, `GridPass.cs:223-225`,
`KSA.Rendering.Lighting/CascadedShadowSystem.cs:308-317`, `Universe.cs:2234`).
...
- **`OrbitView` must be non-null** — `OrbitController.cs:477` dereferences it unguarded, and
  `FixedController.cs:36` does too. Return a stable instance, not a new one per call.
- **`MeanRadius` must be > 0 and finite.** It is a divisor at `OrbitController.cs:513`/`:580`, a
  multiplier at `:593` and `MapController.cs:270`/`:97`, and is narrowed through `float` at
  `Camera.cs:601`.
- `GetPositionEcl()` is called several times per frame from different places — make it cheap and
  **consistent within a frame** (`Camera.cs:116`, `:124`, `FixedController.cs:44`,
  `MapController.cs:273`, `CascadedShadowSystem.cs:313`).
...
- Call `SetFollow(target, tidalLocking, changeControl: false, alert: false)` — the default
  `changeControl: true` sets `Program.ControlledVehicle = target as Vehicle` = null (`Camera.cs:611`).
- **You own the lifetime.** Nothing cleans up `Camera.Following` when a mod object dies; the engine's
  own cleanup (`Universe.cs:1710-1720`) is keyed on `Vehicle` destruction only. Call
  `Camera.Unfollow(false)` or re-`SetFollow` before dropping the reference.
- It will not survive a save/load (`Camera.DeserializeSave`, `Camera.cs:816-833`).
...
- `CameraOffset` is added directly to `Following.GetPositionEcl()` in Ecl axes
  (`FixedController.cs:44`). `GetFrame2Ecl` has no effect on position.
- `GetFrame2Ecl(focused, referenceFrame)` produces one quaternion whose **+Z axis alone** is used, as
  the roll reference for the look-at (`FixedController.cs:37-38`, `:42`). Nothing else from it is used.
- The reference frame comes from `Following.OrbitView.ReferenceFrame` — a field on the *followed
  ...
  (`Vehicle.cs:2926-2928`). For a mod-supplied `IFollowable` the mod controls it; for a `Vehicle` it
  does not.
- Frames available in `FixedController.GetFrame2Ecl` (`:50-100`): for a `Vehicle` —
  ...
  (CCI), `Stars`. A `VehicleEditingSpace` gets its `Asmb2Ecl` flipped 180° about X (`:97`); everything
  else → Identity, with the reference frame not read at all.
  `OrbitController.GetFrame2Ecl` (`OrbitController.cs:214-315`) has the same list plus `Editor`,
  `WreckageMarker` support (`:275-309`), and `HasValue` guards; `FlyController.GetFrame2Ecl` throws for
...
|---|---|---|---|
| 1 | `FixedController.cs:40` | `CameraRotation` left at `double3.Zero` | `DivideByZeroException` via `Double3Ex.cs:20` → `double3.cs:722-727` |
| 2 | `FixedController.cs:40`, `:42` | `CameraRotation` parallel to `frame2Ecl`'s +Z (zenith/nadir, or Ecl +Z under `Stars`) | `DivideByZeroException` |
| 3 | `Camera.cs:200-202` (`LookAtRotation`) | zero forward, or forward parallel to the supplied up | `DivideByZeroException` |
| 4 | `FixedController.cs:58` | `Surface` frame on a vehicle at a parent's centre or exactly on its rotation axis | `InvalidOperationException` (`Nullable.Value`) |
| 5 | `FixedController.cs:62-63` | `Orbit` frame on a vehicle with ~zero CCE velocity (landed) or radial ∥ velocity | `InvalidOperationException` |
| 6 | `Viewport.cs:328` | a `CameraMode` value outside 0–4 written into the public `Mode` field | `ArgumentOutOfRangeException`, every frame |
| 7 | `OrbitController.cs:477`, `FixedController.cs:36` | a mod `IFollowable` returning null `OrbitView` | `NullReferenceException` |
...
| 15 | `Program.cs:3458` (**View → Add Camera**) | a **fourth** camera window. No mod needed — see below. | `InvalidOperationException` ("Sequence contains no matching element") |

**#15 needs no mod at all, and a player can reach it in three clicks.** The guard counts something
different from what the search looks for:
...
`ViewportCount` is a fixed 6 (`Program.cs:249`) and three of the six are offscreen — the thumbnail
(`Program.cs:908`) and the two crew portraits (`:912-928`). `VisibleViewportCount` does now exclude
offscreen viewports (`Program.cs:466`), so it can never exceed 3, and `3 < 6` offers the menu item
however many windows are open. With the main view plus two added cameras every non-offscreen viewport
is visible, so `First` matches nothing and throws. There is no try/catch in the frame loop, so the
process dies.

**So the usable ceiling is three windows: the main view and two others.** `DockingPort.cs:183`
does the same search and gets it right by testing both flags. The fix upstream is to compare
`VisibleViewportCount` against the number of non-offscreen viewports, or to use `FirstOrDefault`
with a null check.
...
- **A mod cannot set the camera's roll in `Fixed` mode — but it can replace the controller.**
  ...
  `Celestial` gets its spin axis or pole, a `VehicleEditingSpace` gets its own assembly frame, and
  **anything else falls to `Identity` with the reference frame not read at all** — so for a mod's own
  `IFollowable` the axis is ecliptic +Z and
...
- **A viewport in `Fixed` mode cannot be recovered by the player from the keyboard or the mouse.**
  ...
  `Viewport.NextCameraMode` (`Viewport.cs:353-369`), whose switch covers Orbit, Free and IVA and
  ...
  (`Program.cs:3444-3455`), and whatever the mod itself offers. **Anything that takes the main
...
1. **Whether `TidalLocking` has any effect.** It is written, saved and displayed, and no reader in the
   corpus changes behaviour on it (`Camera.cs:160`, `:604`, `:835-838`; consumers at `Program.cs:3461`,
   `DockingEvent.cs:23`, `Universe.cs:2186`, `MapController.cs:255` all just pass it through). Settle by
...
3. **Whether `Vehicle.GetPositionEcl()` and the drawn position ever disagree by enough to matter for a
   camera.** `GetPositionEcl` is the analytic value (`Vehicle.cs:2456-2461`) and `Camera.GetPositionEgo`
...
4. **Whether `KittenEva` instances actually carry `IVASeat` modules** in shipped content. The class
   supports parts but the part templates are game content XML, which is not in the decompiled corpus.
   Settle by inspecting `Content/Core/**` in the install.
5. **Whether writing `Viewport.Visible = true` on viewports 2/3 is enough**, or whether something else
   (a render target rebuild, `NewSize`) must happen first. `DockingPort.cs:187-188` sets `Visible` and
   `NewSize` together and the rebuild happens in `OnFrameViewports` (`Program.cs:2420-2429`); whether
...
6. **What happens to a `Fixed` viewport when the game is paused or time-warping.** `Viewport.OnFrame`
   gets `dtPlayer` (`Program.cs:2432`) and `FixedController.OnFrame` does not use `dt` at all, so it
   should keep tracking — but `Following.GetPositionEcl()` is only refreshed when the solvers run
   (`Program.cs:2012`, `:2047`). Unverified in flight.
7. **`Camera.NoRotation` semantics for `Fixed`.** Nothing sets `BaseCamera.NoRotation = true` except
   `MapController` on `MapCamera` (`MapController.cs:556`), and `OrbitController` clears it every frame
...
8. **Whether the StarMap loader offers any hook earlier than the `Program.OnFrame` postfix.** The
   ...
   `OnDrawUiFrame`, which is **after** `OnFrameViewports` (`Program.cs:2091` vs `:2095`), so it is no
...
`Program.OnFrame(currentPlayerTime, dtPlayer)` (`KSA/KSA/Program.cs:2066`) in order:

| Step | Line |
| --- | --- |
| `OnFrameViewports(dtPlayer)` — per-viewport controller + camera + audio | `Program.cs:2091` |
| `OnDrawUiFrame` / `OnDrawUiViewports` (ImGui) | `Program.cs:2095`, `2096` |
| `OnFrameController(dtPlayer)` | `Program.cs:2143` |
| `LightSystem.OnFrame(FrameViewport, dtPlayer)` | `Program.cs:2145` |
| `Cursor.UpdateInputRay(GetCamera())` | `Program.cs:2146` |
| `OnFrameCelestials(dtPlayer)` | `Program.cs:2148` |
| `OnPreRender(dtPlayer)` | `Program.cs:2151` |
| `Render(dtPlayer, …)` → `UpdateShaderData` per viewport, `UpdateRenderingResources`, then `RenderGame` or `RenderEditor` | `Program.cs:2252-2289` |
| `PostRender(dtPlayer)` | `Program.cs:2159` |

`RenderGame` (`Program.cs:4304`) is the whole GPU frame. It renders **secondary** viewports first, in a loop
...
for (int i = 1; i < ViewportCount; i++) {
    ...
    if (viewport.Index != _mainViewportIndex && viewport.Visible)
        RenderViewport(commandBuffer2, viewport, resourceFrameIndex);   // Program.cs:4332-4339
}
_renderedViewportIndex = _mainViewportIndex;                            // Program.cs:4341
… ~250 lines of main-viewport-only passes, inline, never factored into a function …
```

`RenderViewport` is `private` (`Program.cs:4168`). `RenderGame` is `private unsafe` (`Program.cs:4304`).
...
| Pass | Line |
| --- | --- |
| `SuperMeshRenderSystem.UseShadows`/`UseLightPrePass` = `viewport.LightMode == Clustered` | `4171-4173` |
| `KittenEva` vehicles `UpdateRenderData`, `ControlledVehicle.UpdateSeatedCrewRenderData` | `4177-4186` |
| `_milkyWayRenderer.RenderMilkyWay` / `_starStarTechnique.Render`, both under `viewport.ShouldRenderStars && GameSettings.ShowStars()` | `4211`, `4217`, `4219` |
| `StaticCelestialDistanceRendering.Run` (point-sprite bodies) | `4222` |
| `StaticCelestial.RenderSphere` for every `StaticCelestial` in the system | `4231` |
| `GenericMeshRenderer.WriteCommands` | `4235` |
| `SplineRenderer.WriteCommands` | `4236` |
| `PartModelRenderer.ColorData.WriteCommands` | `4237` |
| `StaticObjectRenderer.WriteCommands` | `4238` |
| `SuperMeshRenderSystem.RenderMainPass` | `4240` |
| `StaticObjectRenderer.WriteCommandsBlended` | `4241` |
| `_sunbloomRenderer.Render` | `4251` |
| `SingleToMultisamplePass.Run` (if MSAA) | `4255` |
| `SuperMeshRenderSystem.RenderTranslucencyPass` | `4262` |
| `OrbitLinePass.Run` (if `DrawUI`) | `4265` |
| `GizmoPass.Run` | `4267` |
| `RenderAaPreparePasses` + `RenderFinalComposite` into `viewport.MainTarget` | `4270-4283` |

That is: **stars, distant-body sprites, distant-body spheres, vehicle meshes, static objects, orbit
lines, part gizmos.** Nothing else. `LightMode` is the one lever on that list a mod can pull: it is a
public field (`Viewport.cs:24`), and only `MainViewport` is set to `Clustered` at boot, so a secondary
viewport gets neither shadows nor the light pre-pass unless a mod asks for them.
...
(`Program.cs:4341`), i.e. strictly outside the per-viewport loop at `4332-4339`:

| Pass | Line |
| --- | --- |
| `PrePassRenderer.Render` (depth/normal pre-pass) | `4361` |
| `AmbientOcclusionRenderer.Render` | `4366` |
| `LightSystem.RenderShadowPass` | `4371` |
| `LightSystem.DispatchComputePasses` | `4374` |
| `_planetRenderer.GenerateMeshData` | `4380` |
| `_planetRenderer.UpdateUvOffsets` | `4381` |
| `_planetRenderer.OnFrameGroundClutter` (hardcoded `MainViewport`) | `4383` |
| `_planetRenderer.UpdateGroundClutter` | `4384` |
| `SunShadowSystem.TerrainShadowMap` + `_planetRenderer.RenderSunShadow` | `4387-4389` |
| `_cascadedShadowSystem` cascades + `_planetRenderer.RenderShadow` | `4395-4416` |
| `_planetTransparenciesRenderer.UpdateRingMeshes` | `4419` |
| `PartSelectedRenderer.ClearColorImage` / `.Run` | `4421`, `4544` |
| `RaytracingRenderer.RunRaytracePass` (IVA only) | `4425` |
| `_clutterPrePass` + `_planetRenderer.RenderDepthPrePass` | `4435-4437` |
| `_sunRenderer.Draw()` | `4464` |
| `_planetTransparenciesRenderer.RenderRingMeshes` | `4479` |
| **`_planetRenderer.Render` — the terrain** | `4483` |
| `_volumetricExhaustRenderer.RenderDebugWireFrame` / `.Render` | `4489`, `4533` |
| **`GizmosRenderer.Render` — the debug line/sphere gizmos** | `4490` |
| `ParticleSystem.UpdateParticles` / `WriteCommandsColorOpaque` / `…Translucent` | `4498`, `4499`, `4562` |
| `_planetRenderer.ProcessExporter` | `4511` |
| **`_oceanRenderer.Render` / `.RenderUnderwater`** | `4514`, `4572` |
| `_transparenciesMsaaResolve` | `4517`, `4535` |
| **`_planetTransparenciesRenderer.Render` — atmosphere + clouds** | `4522` |
| `_overallBloomRenderer.Render` | `4542` |
| `PartModelGlass.Shared.WriteCommandsColor` | `4553` |
| `GridPass.Run` | `4576` |
| `GaugeCanvas.CanvasesRender` | `4580` |
| `ImGauge.PrepareRender` | `4584` |
| `_screenshotCapture.OnRenderGameCapture` / `OnRenderGameSwapchainGrab` | `4587`, `4593` |

So: **no terrain, no atmosphere, no clouds, no ocean, no ambient occlusion, no particles, no exhaust,
no bloom (other than sun bloom), no `GizmosRenderer` lines/spheres, no gauges, no screenshot capture**
in a secondary viewport. Shadows and the light pre-pass are the exception the `LightMode` field opens
(A.1), and the terrain and cascade shadow maps are fitted to the main camera anyway (A.5).

Note in particular for a mod that draws diagnostics with `GizmosRenderer.DrawLine`/`DrawSphere`:
`GizmosRenderer.Render` (`KSA/KSA/GizmosRenderer.cs:338`) is called only at `Program.cs:4490`
(game) and `Program.cs:4688` (editor). Those primitives are invisible in a secondary viewport.
`GizmoPass` (`Program.cs:4267`) is a different subsystem — the part-manipulation gizmos in
...
(`Program.cs:4094`). Inside it (`KSA/KSA/StaticCelestialDistanceRendering.cs:400-443`):

```
if (!(astronomical is IOrbiter orbiter) || camera.NearbyCelestial == orbiter || …) continue;   // :416
…
if (num4 > 2.0 && staticCelestial != null)
    staticCelestial.RenderSphereThisFrame[viewport.Index] = true;                              // :434-436
```
...
for `RenderSphere`. `StaticCelestial.RenderSphere` (`KSA/KSA/StaticCelestial.cs:33-42`) then invokes
`DistantSphereRenderer.Render` (`KSA/KSA/DistantSphereRenderer.cs:74`), which draws:

- a `ModLibrary.Get<MeshReference>("Sphere")` (`StaticCelestial.cs:22`)
- scaled to `_celestial.MeanRadius` — the **mean** radius, no heightfield (`DistantSphereRenderer.cs:78`)
- textured from a streamed low-res diffuse/normal cubemap (`DistantSphereRenderer.cs:98-99`)
- with `VkCullModeFlags.BackBit` and depth test+write (`DistantSphereRenderer.cs:118`)
...
`Camera.NearbyCelestial` is a plain `{ get; set; }` (`KSA/KSA/Camera.cs:71`). The **only** places it is
assigned in the whole engine are `Program.OnFrameCelestials`:

```
private void OnFrameCelestials(double deltaTime) {
    Camera camera = GetCamera();                       // Program.cs:2482
    …
    camera.NearbyCelestial = null;                     // Program.cs:2490  (no billboarded body)
    camera.NearbyCelestial = astronomical as Celestial; // Program.cs:2493
    …
    camera.NearbyCelestial = null;                     // Program.cs:2509  (>80 000 km from surface)
}
```

`GetCamera()` is `FrameViewport.GetCamera()` (`Program.cs:599-602`), and `FrameViewport` is
`Viewports[_frameViewportIndex]` (`Program.cs:476`). At the moment `OnFrameCelestials` runs
(`Program.cs:2148`), `_frameViewportIndex` has been reset to `_mainViewportIndex` by `PrepareFrame`
(`Program.cs:2034`) and by the previous frame's `UpdateRenderingResources` (`Program.cs:4030` and again
`4047`), and both `Render`'s `UpdateShaderData` loop (`Program.cs:2269-2273`) and `OnDrawUiViewports`
(`Program.cs:2923`, `2932`) move it and put it back. `_mainViewportIndex` is assigned exactly once, to
`0`, at boot (`Program.cs:903`) and never again.
...
in `CameraMode.Map` and `BaseCamera` otherwise (`Viewport.cs:332-339`), so even on the main viewport only
...
(`Program.cs:4480-4484`), the ocean (`4506-4515`, `4569-4573`) — would therefore be skipped for a secondary
...
| --- | --- | --- |
| `NearbyCelestial` | `Program.cs:2490-2509`, frame viewport only | above |
| `DistanceToNearbyCelestialKm`, `…SurfaceMeanKm`, `NearbyCelestialTerrainHeight`, `CurrentAltitudeKm` | `Program.cs:2496-2503`, same block | stay 0 |
| `_sunPositionEgo`, `_sunRenderer.UpdateShaderData` | `Program.cs:2664-2668`, guarded `viewport.Index == _mainViewportIndex` | no sun disc |
| `BiomeSoundController` | constructed only for `index == 0` (`Viewport.cs:122-125`) | no environment audio |
| IVA audio blend | early-returns for `Index != 0` (`Viewport.cs:162-165`) | — |
| `SunShadowSystem.UpdateUniforms` / `_cascadedShadowSystem.UpdateUniforms` | `Program.cs:4048-4049`, against `RenderedViewport` which is main at that point | shadow maps are main-camera-fitted |

Planet lighting is **not** on that list any more. `UpdateShaderData` falls back to the main camera's
nearby body when a secondary camera has none (`Program.cs:2626-2630`), so
`UpdatePlanetShaderData(nearbyCelestial, viewport)` (`Program.cs:2765`) writes a real
`PlanetColor`/`PlanetRadius`/`AtmosphereLutLayer`/`OcclusionColor` per viewport rather than the
`float4.Zero` of its `else` arm (`:2785`, `:2798`). A secondary viewport is lit by the planet the *main*
camera is near, which is right whenever both cameras are at the same body and wrong otherwise.
...
every viewport at `Program.cs:2269-2273`. `GlobalShaderBindings` reserves a per-viewport slice of the
uniform buffer and `DynamicOffset(viewportIndex)` selects it (`KSA/KSA/GlobalShaderBindings.cs:55-66`).
...
1. **The calls are hardcoded and unreachable.** `_planetRenderer`, `_oceanRenderer`,
   ...
   exposes `GetPlanetRenderer()` (`Program.cs:540`) and `GetOceanRenderer()` (`Program.cs:550`) publicly,
...
2. **The renderers are bound to a single render target at construction.** `PlanetRenderer` takes an
   `IRenderPassInfo` in its constructor (`KSA/KSA/PlanetRenderer.cs:462`) and is built with
   `_offscreenTarget` (`Program.cs:1122`); so are `_planetTransparenciesRenderer` (`1128`),
   `_volumetricTrailRenderer` (`1139`), `_oceanRenderer` (`1143`), `_overallBloomRenderer` (`1152`),
   `GizmosRenderer` (`1069`), `_volumetricExhaustRenderer` (`1065`), `_transparenciesMsaaResolve` (`1061`).
   `_offscreenTarget` is the **main viewport's** target: `MainViewport.OffscreenTarget = _offscreenTarget`
   (`Program.cs:1477`), sized to `MainViewport.Size` (`Program.cs:1462`). `RenderTechnique.CreatePipeline`
   bakes that pass info into the pipeline (`KSA/RenderCore/RenderTechnique.cs:114-117`). Calling them with
   ...
   `_renderer.DepthFormat` and `GameSettings.GetSampleCount()`, `Viewport.cs:203-211` — so the obstacle is
...
3. **The supporting resources are singletons sized to the main viewport / swapchain.**
   `PrePassRenderer`, `AmbientOcclusionRenderer`, `_clutterPrePass` (`Program.cs:1471`),
   `GizmoPass.ColorImage`/`DepthImage` (built at `renderer.Extent`, `KSA/KSA/GizmoPass.cs:73-74`),
...
`Program.cs:4590`), and `_compositeRenderer[0]` is wired to `_offscreenTarget.ColorImage`
(`Program.cs:4776-4778`).

**The one thing that would change what a secondary viewport shows, cheaply:** the grey ball is drawn
because `camera.NearbyCelestial != orbiter` (`StaticCelestialDistanceRendering.cs:416`). `NearbyCelestial`
is a public settable property (`Camera.cs:71`). Assigning the home body to the secondary camera each frame
...
`SampledBit | StorageBit | ColorAttachmentBit` (`RenderTarget.cs:114`), so they can be sampled and
written by compute. `ColorImage`, `DepthImage`, `BeginRendering`, `EndRendering`, `ResolveAttachments`,
`Rebuild`, `SetupGraphicsPipeline` are all public (`RenderTarget.cs:34-48`, `173-360`).

`Program.GetRenderer()` (`Program.cs:535`) and `Program.GetRendererContext()` (`Program.cs:4884`) are
public statics; `RendererContext` carries `Renderer`, `IRenderPassInfo`, `FrameCount`
(`KSA/KSA/RendererContext.cs:6-12`). `Program.OffscreenTarget` (`Program.cs:438`) and
`Program.MainPass` (`Program.cs:436`) are public statics too, so a mod can *read* the main colour image.
...
(`Viewport.cs:213-218`), drawn with `ImGui.ImageWithBg` (`Viewport.cs:270`).
...
Options)` using `Brutal.ShaderCApi.CompileOptions` (`KSA/KSA/ShaderReference.cs:109`). It resolves
`#include`s through a callback and records the include paths for hot reload
(`ShaderReference.cs:140-166`). Stage is inferred from the file extension
(`ShaderReference.cs:50`). There is a `CompileVariantWithCustomOptions(CompileOptions)` public method
(`ShaderReference.cs:120`). `Brutal.ShaderC.dll` ships with the game (`current/dll/Brutal.ShaderC.dll`).

`ShaderReference` is a first-class asset type: `[XmlElement("Shader", typeof(ShaderReference))]` in the
asset bundle schema (`KSA/KSA/AssetBundle.cs:22`), alongside `[XmlElement("GaugeShader", …)]` (`:23`).
It derives from `FileReference` and resolves `ModPath` against the owning mod
(`ShaderReference.cs:45-52`, `:87-98`). **A content mod can therefore ship its own `.vert`/`.frag`/`.comp`
...
`Program.DisableHotShaders()` (`Program.cs:4909`).

`RenderTechnique` is `public abstract` with a primary constructor taking `(name, Renderer,
IRenderPassInfo, Span<ShaderReference>, Span<VkSpecializationInfo>)` and protected `CreatePipeline`
helpers (`KSA/RenderCore/RenderTechnique.cs:13`, `:59-67`). Subclassing it from a mod is possible; the
only abstract members are `MakeVertexInput()` and `OnRebuildFrameResources()`
(`RenderTechnique.cs:130`, `:138`).
...
| --- | --- | --- |
| `StarMapBeforeGui` | `void(double)` | Prefix on `Program.OnDrawUiFrame` (`StarMap.Core/StarMap.Core.Patches/ProgramPatcher.cs:18-20`) |
| `StarMapAfterGui` | `void(double)` | Postfix on `Program.OnDrawUiViewports` (`ProgramPatcher.cs:29-31`) |
...
(`Program.cs:2159`). So a mod cannot inject a pass into KSA's frame graph through the loader API.
...
- create a `VkCommandPool` from `renderer.Device.CreateCommandPool` (`:41`)
- drive a `Viewport`'s camera by hand and call `Program.Instance.UpdateShaderData(dt, viewport)` and
  `Program.Instance.UpdateRenderingResources(0)` — **both public** (`:70-71`,
  `Program.cs:2619`, `Program.cs:4026`)
- build a pipeline from `ModLibrary.Get<ShaderReference>` modules (`ThumbnailRenderer.cs:63-76`)
- allocate a command buffer, record, `SubmitOneTimeRenderCommand`, wait on a fence (`:98-113`, `:235`)
- render into an image created with `ColorAttachmentBit | SampledBit | TransferSrc/DstBit` (`:166`)

It also renders through `GlobalShaderBindings.DescriptorSetLayout` + `DynamicOffset(viewport.Index)`
(`ThumbnailRenderer.cs:53-57`), i.e. a mod's own pipeline can consume KSA's per-viewport camera UBO.
...
- **A full-screen colour grade of the existing main view: possible in principle but with no clean hook.**
  ...
  composite — `RenderFinalComposite` (`Program.cs:4291-4302`) reads `_compositeRenderer[viewportIndex]` /
  `_cmaa2Renderers[…]`, both private arrays (`Program.cs:147`, `:149`).
...
```
public static int ViewportCount = 6;                                       // Program.cs:249
public static readonly List<Viewport> Viewports = new List<Viewport>();    // Program.cs:251
```

`using (Loading.Task("Build Viewports"))` (`Program.cs:900-931`):

| Index | Created as | Size | Own render targets |
| --- | --- | --- | --- |
| 0 | `_mainViewportIndex = 0` (`:903`), `LightMode = Clustered` (`:905`), `Visible = true` (`:930`) | swapchain extent | **no** — `buildRenderTarget: false` (`:904`); later `MainViewport.OffscreenTarget = _offscreenTarget` (`:1477`), `MainTarget` stays null and it composites straight to the swapchain |
| 1 | `_thumbnailViewportIndex = 1` (`:906`), `IsOffscreen = true`, `ShouldRenderGizmos = false`, `LightMode = None` (`:908-910`) | `ThumbnailRenderer.SIZE` | yes |
| 2, 3 | loop `for (num4 = 2; num4 < 4; num4++)` (`:912-914`) | 500×500 | yes |
| 4, 5 | crew portraits, `_crewPortraitViewportStart = 4` (`:915`); `IsOffscreen`, `Visible`, `Mode = Fixed`, no stars, no resize, 42° (`:916-928`) | 128×128 | yes |

`AddViewport` (`Program.cs:629-646`) is `private`. Each viewport gets **two** cameras — `BaseCamera` and
`MapCamera` — plus five controllers: `FlyController`, `OrbitController`, `MapController`, `IVAController`,
`FixedController` (`Viewport.cs:109-127`).

**So there are exactly two viewports available to a mod: indices 2 and 3.** `VisibleViewportCount`
(`Program.cs:466`) counts `Visible && !IsOffscreen`, and the "Add Camera" menu item picks
`Viewports.First(v => !v.Visible && !v.IsOffscreen)` (`Program.cs:3456-3459`) — see §7 #15 for why those
two do not add up. `DockingPort` does the same scan (`KSA/KSA/DockingPort.cs:181-192`).

`ViewportCount` is a public mutable static, but raising it after boot is not viable: `_compositeRenderer`
(`Program.cs:1080`), `_cmaa2Renderers` (`:1101`), `_cameraData`, `_lightingData`, `_celestialData`,
`_vesselData` (`:1154-1157`), `StaticCelestial.RenderSphereThisFrame` (`StaticCelestial.cs:15`) and the
`GlobalShaderBindings` uniform buffer (`GlobalShaderBindings.cs:94`) are all sized from it at
initialisation, and `_sunflareData` (`Program.cs:1052`) from `Viewports.Count`.

#### C.2 Destruction

`Viewports` is never cleared and no viewport is destroyed at runtime. `Viewport.Dispose` is called only
from `Program.Dispose` at shutdown (`Program.cs:1328-1330`), and it only frees render targets when
`_ownsRenderTargets` (`Viewport.cs:129-143`) — which is set by `BuildRenderTarget()` (`Viewport.cs:205`),
so true for 1–5 and false for 0.

"Closing" a viewport is `Visible = false`. `Viewport.DrawImGui` binds `ref Visible` to
`ImGui.Begin(_viewportName, ref Visible, flags)` (`Viewport.cs:259`), so the window's X button clears it.
`DockingPort.ResetDockingViewport` shows the intended teardown: `Visible = false`,
`SetCameraMode(CameraMode.Orbit)`, `AllowResize = true`, `DockingConnector = null`
(`DockingPort.cs:133-142`), called from `DockingPort.Dispose` (`:130`).

`RebuildViewport(viewport)` (`Program.cs:4725-4740`) does `Device.WaitIdle()`, resizes the viewport's
...
when `NewSize` differs or the sample count changed (`Program.cs:2420-2429`).

#### C.3 `MainViewport` vs `Viewports`

`MainViewport => Viewports[_mainViewportIndex]` (`Program.cs:468`) — **it is element 0 of the list**, not a
separate object. Same for `HoveredViewport` (`:470`), `RenderedViewport` (`:472`), `ThumbnailViewport`
(`:474`), `FrameViewport` (`:476`). `_mainViewportIndex` is private and only ever `0` (`Program.cs:903`).

#### C.4 What happens to a camera a mod has moved

| Event | Effect | Cite |
| --- | --- | --- |
| **Vehicle destroyed** (`Universe.DestroyVehicleFromEvent`) | loops **all** viewports; any `BaseCamera` following the vehicle is switched to `FollowWreckage(vehicle, cause)`; any `MapCamera` following it is re-followed onto `vehicle.Parent` | `KSA/KSA/Universe.cs:1710-1720` |
| `FollowWreckage` | creates a `WreckageMarker` and calls `SetFollow(marker, tidalLocking: false)` — which also sets `Program.ControlledVehicle = target as Vehicle` (null here) because `changeControl` defaults true | `Camera.cs:590-595`, `:597-613` |
| **Save loaded** (`Universe.DeserializeSave`) | loops all viewports, `BaseCamera.Unfollow()` and `MapCamera.Unfollow()` | `Universe.cs:2106-2110` |
| `Unfollow(changeControl: true)` | clears `_following`, `_wreckageMarker`, `_tidalLocking`, and sets `Program.ControlledVehicle = null` | `Camera.cs:615-626` |
| **Camera mode change** | `SetCameraMode` calls `OnSwitchOff`/`OnSwitchOn` on the controllers and `Program.ControlledVehicle?.ClearHeldPlayerInput()` | `Viewport.cs:341-351` |
| **Quit** | `Program.Dispose` → `viewport.Dispose()` | `Program.cs:1328-1330` |
...
(`Viewport.cs:145-150`). `FixedController.OnFrame` recomputes `PositionEcl` and `LocalRotation` from
`Camera.Following`, `CameraOffset` and `CameraRotation` every frame (`KSA/KSA/FixedController.cs:22-48`)
...
(`Viewport.cs:353-369`), so the player cannot rotate out of it with the camera key.

#### C.5 Camera state in the save

`UniverseData.Create()` serialises **only `Program.MainViewport.BaseCamera`**
(`KSA/KSA/UniverseData.cs:44-49`). `Camera.SerializeSave` writes `Program.MainViewport.Mode`,
`LocalPosition`, `LocalRotation`, the followed object's Id, tidal locking, and the map controller's
inverted flag / previously-controlled vehicle (`Camera.cs:801-814`). `CameraData` is a field of
`UniverseData` (`UniverseData.cs:13-14`), which is the fixed XML-mapped save class.
...
`Camera.DeserializeSave` will restore that mode and follow target on load (`Camera.cs:816-833`).
...
- **Free** = `FlyController`, whose own private `ClampCamera` reads `Program.GetNearbyCelestial()` and
  `Program.GetCurrentAltitudeKm()` (`KSA/KSA/FlyController.cs:859-860`) — globals, not per-viewport, so a
  secondary free camera is floored against the *main* camera's nearby body. Its speed scaling is
  self-contained (`GetCurrentSpeed`, `:118-131`).
- **Fixed** = `FixedController` — position `Following.GetPositionEcl() + CameraOffset`, orientation from
  ...
  (`FixedController.cs:22-48`, `:50-100`). The docking-port camera uses the same controller through its
  own `DockingConnector` branch (`DockingPort.cs:189-191`); the two `double3` fields are the natural
  route for a mod-driven optical head.
- **Orbit zoom is camera distance, not FOV**: `OrbitView.DistancePower` (`KSA/KSA/OrbitController.cs:381-411`).
  **`OrbitView` belongs to the followed object, not to the viewport** — `IFollowable.OrbitView`
  (`KSA/KSA/IFollowable.cs:5`), `Astronomical.OrbitView` (`KSA/KSA/Astronomical.cs:60`, `:114`). Two viewports
...
- Near plane `0.1f`, far plane `1.4959786E+11f` (1 AU), ortho far `50000f` — all `private const`
  with public read-only accessors (`Camera.cs:29-35`, `:67-69`). **Not settable.** Reverse-Z projection
  (`Camera.UpdateProjection`, `Camera.cs:466-480`).
- FOV: `_fovRadians`, default `0.87266463f` = 50° (`Camera.cs:53`). `SetFieldOfView(float fovDegrees)`
  (`Camera.cs:412-416`) is **unclamped**; `ChangeFieldOfView(float)` clamps to `[15, 120]`
  (`Camera.cs:450-457`, constants at `:37-39`). `GetFieldOfView()` returns **radians** (`Camera.cs:774-777`).
  `GameSettings.ApplyTo(Camera)` calls `SetFieldOfView(FieldOfView)` (`KSA/KSA/GameSettings.cs:2415-2418`),
  and `AddViewport` applies it to both cameras at creation (`Program.cs:634`, `:638`).
- `SetOrthographic(bool)` / `SetOrthoHalfHeight(float)` are public (`Camera.cs:418-438`).

#### D.3 Screenshots

`ScreenshotCapture` (`KSA/KSA/ScreenshotCapture.cs`), registered as a terminal command
`Screenshot(int, string)` (`Program.cs:1486`, `:1585`). Flags `ui`, `hud`, `warm=N`
(`ScreenshotCapture.cs:105-136`). Two modes: a swapchain grab (`ui`, includes ImGui) or a supersampled
offscreen capture, scale clamped to `[1, 8]` and further reduced against
`MaxImageDimension2D`/`MaxFramebuffer*` limits and free device memory (`:141-180`). Files go to
`Documents/exports/screenshots` (`:80`). It hooks the frame at
`_screenshotCapture.OnRenderGameCapture` (`Program.cs:4587`) and `OnRenderGameSwapchainGrab`
(`Program.cs:4593`) — **main viewport only**; there is no per-viewport capture. It drives
`Program.CompositeViewportSizeOverride` (`ScreenshotCapture.cs:330`, cleared at `:212`, `:419`, `:446`),
which `Program.SetViewport` honours (`Program.cs:4151`) — a public static `int2?` a mod could set and
...
1. **`Camera.ClampCamera` reads the *frame* viewport's state, not its own.** It runs from
   `Camera.OnFrame` (`Camera.cs:484`) for **every** camera, but tests
   `Program.GetNearbyCelestial()` and `Program.GetCurrentAltitudeKm()`
   (`Camera.cs:628-642`), both of which resolve through `FrameViewport` (`Program.cs:599`, `4859-4862`,
   `2513-2524`). During `OnFrameViewports` the frame index is the main viewport, so when the **main**
...
2. **`Camera.SetFieldOfView` throws for out-of-range values.**
   ...
   when `fieldOfView <= 0 || >= π` (`Planet.Render.Core/RenderCore/ReverseDepthBufferUtils.cs:8-13`),
   reached from `UpdateProjection` (`Camera.cs:474`). `SetFieldOfView` does not clamp
   (`Camera.cs:412-416`), so `SetFieldOfView(0)` or `SetFieldOfView(180)` throws out of a mod's frame hook.
3. **Zero-height viewport ⇒ non-invertible projection.** `Camera.Resize` computes
   `AspectRatio = X / Y` with no guard (`Camera.cs:459-464`); `UpdateProjection` then fails
   `float4x4.Invert` and throws `Exception("Tried to assign non-invertible projection matrix")`
   (`Camera.cs:476-479`). `Viewport.SetSize` calls `Resize` on both cameras (`Viewport.cs:300-306`), and
   `OnFrameViewports` only guards against `NewSize == int2.Zero` — **both** components zero
   (`Program.cs:2422`). A `NewSize` of `(500, 0)` passes the guard.
4. **`Universe.DestroyVehicleFromEvent` will steal a mod's camera.** It iterates every viewport and
   re-follows any camera pointed at the destroyed vehicle (`Universe.cs:1710-1720`), and `SetFollow`
   defaults `changeControl: true`, which writes `Program.ControlledVehicle` (`Camera.cs:609-612`).
5. **Making an offscreen viewport visible burns a full render.** `RenderGame`'s loop tests
   only `viewport.Index != _mainViewportIndex && viewport.Visible` (`Program.cs:4335`), while
   `Viewport.DrawImGui` early-returns on `IsOffscreen` (`Viewport.cs:241-246`). Setting
   `ThumbnailViewport.Visible = true` renders it every frame and never shows it — and the two crew
   portrait viewports ship in exactly that state, `IsOffscreen` and `Visible` together (`Program.cs:924-926`),
   which is what `GameSettings.ShowCrewPortraitCameras()` turns off (`Program.cs:2454-2464`).
6. **`GizmoPass` writes into a swapchain-sized shared image for every viewport.** `GizmoPass.ColorImage`
   and `DepthImage` are built at `renderer.Extent` (`GizmoPass.cs:65-84`) and are a single static pair
   (`GizmoPass.cs:15-17`), reused across viewports; `Program.SetViewport` sets the Vulkan viewport to
   `RenderedViewport.Size` (`Program.cs:4148-4166`). Not a mod-reachable crash, but it means gizmo-pass
...
(`Viewport.cs:262-263`), and both are public fields (`Viewport.cs:32-34`). Inside the window it calls
`Universe.OnDrawUi(this)` (`Viewport.cs:288`, → `Universe.cs:1600-1603`) and
`GetCamera().NearbyCelestial?.DrawUiNearby(...)` (`Viewport.cs:289` — a no-op for a secondary viewport,
per A.4). StarMap's `StarMapAfterGui` is a **postfix** on `OnDrawUiViewports` (`ProgramPatcher.cs:29-31`),
...
restores it afterwards (`Program.cs:2921-2933`) — so `Program.GetCamera()` inside a viewport's own UI
refers to that viewport, but inside a StarMap `AfterGui` hook it is back to the main viewport.

#### D.6 Cursor rays

`Cursor.UpdateInputRay(GetCamera())` uses the frame (main) camera once per frame (`Program.cs:2146`).
Per-viewport picking must use `viewport.GetCamera().ScreenToEgoRay(...)` directly
(`Camera.cs:688-708`), which divides by that camera's own `FramebufferSize`
(`Camera.ScreenToEgoNearPlane`, `Camera.cs:658-686`). `Camera.FramebufferSize` is set by
`Camera.Resize` from `Viewport.SetSize` (`Camera.cs:459-464`, `Viewport.cs:300-306`), i.e. it equals
...
- **Whether the secondary path's atmosphere-LUT barriers imply any sky contribution.** `RenderViewport`
  ...
  `SkyTransmittanceGb` (`Program.cs:4193-4195`, `4245-4247`) and runs `_sunbloomRenderer.Render`
  (`:4251`), which samples them. Whether any visible sky tint results, or only the sun-flare/bloom effect,
...
- **Whether `Brutal.ShaderC` compiles at load or caches SPIR-V to disk.** `ShaderReference.Compile` calls
  `ShaderModuleUtils.FromFile` (`ShaderReference.cs:109`); whether `FromFile` has a pipeline/SPIR-V cache
...
- **Whether a mod's `<Shader>` element actually survives `ModLibrary.LoadAll()` from a non-Core mod.**
  The XML element exists (`AssetBundle.cs:22`) and `ShaderReference` resolves `ModPath` against its owning
  `Mod` (`ShaderReference.cs:87-98`). `ModLibrary.LoadAll` / `DoLoad` ordering is untraced, so whether
...
- **Whether `0Harmony.dll` is redistributed with StarMap or expected beside it.** `StarMap.Core` uses
  `HarmonyLib` (`ProgramPatcher.cs:3`), and `0Harmony.dll` is now in the mirrored loader DLL set
  (`current/dll/`) and in `ksa-assemblies.lock`. Whether the loader ships it or only expects it beside
  itself is still only settled by checking a real StarMap install directory.
- **Exact pixel threshold at which the distant sphere replaces the sprite for the home body.**
  ...
  (`StaticCelestialDistanceRendering.cs:434-438`); `SpriteSphereBlend` governs the crossfade
  (`:314`, `DistantSphereRenderer.cs:83`), and its body is unread.
- **What `RenderedViewport` is during `UpdateRenderingResources`'s `SunShadowSystem.UpdateUniforms` /
  `_cascadedShadowSystem.UpdateUniforms` calls (`Program.cs:4048-4049`).** By inspection it is the main
  viewport (`RenderGame` restores `_renderedViewportIndex = _mainViewportIndex` at `:4341` and
  `UpdateRenderingResources` runs before `RenderGame`), but `_renderedViewportIndex` is also written by
  `RenderViewport` (`:4170`) and the very first frame is untraced. It does not change any conclusion.
