...
def check_launcher_geometry():
    ...
    for field, want in scalars:
        ...
        if match is None:
            print(f"  MISSING LauncherPart.{field}", file=sys.stderr)
            problems += 1
        elif abs(float(match.group(1)) - want) > 5e-4:
            print(f"  STALE LauncherPart.{field} = {match.group(1)}, "
    ...
    for field, key in (("PodPivotFromTurret", "pod_pivot_from_turret"),
                       ("TurretPivotInPart", "turret_pivot"),
        ...
        if match is None:
            ...
            if any(abs(a - b) > 5e-4 for a, b in zip(got, expected[key])):
                print(f"  STALE LauncherPart.{field} = {tuple(got)}, "
                      f"mesh says {tuple(expected[key])}", file=sys.stderr)
                problems += 1

    found = [tuple(float(v) for v in m)
             for m in re.findall(r"new\(\s*(-?[\d.]+),\s*(-?[\d.]+),\s*(-?[\d.]+)\s*\),", text)]
    want = [tuple(t) for t in expected["tubes"]]
    checked += len(want)

    if found != want:
        print(f"  STALE LauncherPart.TubeOffsetsPartFrame: {len(found)} entries, "
...
def main():
    if not CORE.is_dir():
        ...
        print("       set KSA_DIR to your install and retry", file=sys.stderr)
        return 1
    ...
    print("checking src/AirDefence/LauncherPart.cs against the mesh")
