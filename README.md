...
Five weapon systems, each a different shape of launcher on the same machinery:
...
far end arrives slow and turns badly. Its art is the first that was *authored* in Blender rather
than generated by a script, so it is unwrapped and its bands are painted rather than modelled.
