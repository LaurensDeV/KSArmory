...
public class InterceptorTests
{
        ...
        double3 frameVelocityEcl = default)
    {
        ...
        while (round.State == RoundState.Flying && t < timeout)
        {
            ...
            round.Update(frameDt, new TargetState(currentTargetPos, targetVel, targetRadius),
                gravity, frameVelocityEcl, platformEcl: default, config);
            t += frameDt;
        }
    ...
    public void ClimbingTarget_IsIntercepted_UnderGravity()
    {
        ...
        var result = Engage(
            launchPos: new double3(0, 0, 0),
            launchVel: new double3(0, 0, config.LaunchSpeed),
    ...
    public void EngagementIsUnchanged_WhenCarriedByAFastMovingFrame()
    {
        ...
        var carried = Engage(
            launchPos: new double3(0, 0, 0),
            launchVel: new double3(config.LaunchSpeed, 0, 0) + frame,
            targetPos: new double3(2500, 0, 0),
    ...
    public void Fuse_StaysSafe_UntilArmed()
    {
        ...
        round.Update(0.1, target, NoGravity, frameVelocityEcl: default, platformEcl: default, config);

        Assert.Equal(RoundState.Flying, round.State);
        Assert.True(round.Age < config.FuseArmSeconds);
    }
    ...
    public void Seeker_BreaksLock_WhenTargetLeavesItsFieldOfView()
    {
        ...
        round.Update(1.0 / 60.0, target, NoGravity, frameVelocityEcl: default, platformEcl: default, config);
    ...
    public void LostTarget_LeavesRoundCoasting_ThenExpires()
    {
        ...
        {
            round.Update(1.0 / 60.0, target: null, NoGravity, frameVelocityEcl: default, platformEcl: default, config);
            t += 1.0 / 60.0;
        }
    ...
    public void GuidanceCommand_IsPerpendicularToFlightPath_AndWithinTheGLimit()
    {
        ...
        double3 command = Interceptor.GuidanceAccel(r, v, missileVelocity, NoGravity, config);
        ...
        Assert.True(Vec.Len(command) <= config.MaxLateralAccel + 1e-6,
            $"command {Vec.Len(command):F1} m/s^2 exceeded the {config.MaxLateralAccel:F1} limit");
    }
