...
/// </summary>
public class BusDeploymentTests(ITestOutputHelper Out)
{
    ...
    private static BusDeployment Started(out double3[] axes, out double3 reference)
    {
        axes = Axes();
        BusDeployment deploy = new();
        Assert.True(deploy.Begin(axes));
        reference = deploy.ReferenceCci;
        return deploy;
    }

    private static BusSituation At(int tube, double3 axisNow, double sweep,
    ...
    {
        BusDeployment deploy = Started(out double3[] axes, out _);

        BusRelease r = deploy.Update(1.0 / 60.0, At(0, axes[0], 0.0));
    ...
    {
        BusDeployment deploy = Started(out _, out double3 reference);

        BusRelease r = deploy.Update(1.0 / 60.0, At(0, reference, sweep: 0.5));
    ...
    public void ItReleasesOnceTheTubeIsOnTheLineAndStill()
    {
        ...
        BusRelease r = deploy.Update(1.0 / 60.0, At(0, reference, sweep: 0.0));
    ...
    public void ABusThatCannotPointReleasesAnywayAndStopsTryingForTheRest()
    {
        ...
        BusRelease next = deploy.Update(1.0 / 60.0, At(1, axes[1], 0.0));
    ...
    public void RunningOutOfWindowStopsRepointingRatherThanHoldingWarheads()
    {
        ...
        BusRelease r = deploy.Update(1.0 / 60.0, At(0, axes[0], 0.0, windowSeconds: 20.0));
    ...
    public void EachTubeGetsItsOwnClock()
    {
        ...
        BusRelease r = deploy.Update(1.0 / 60.0, At(1, axes[1], 0.0));
        Assert.False(r.ReleaseNow);
    }
    ...
    {
        BusDeployment deploy = Started(out _, out _);

        BusRelease r = deploy.Update(1.0 / 60.0, At(-1, Vec.Zero, 0.0));
