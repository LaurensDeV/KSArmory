...
| `Sim/ReleaseWalker.cs` | the cursor that walk is flown through, and the three numbers the flight reads off it — **the gate, the stop's quota and which target a warhead is going to**. Each hands back what the flight used before it existed while `Walking` is false, which it is for every set of one; and the plan is **committed by the first warhead leaving**, because re-ordering stops behind a bus aims it at a place it has been |
