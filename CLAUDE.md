...
| `Sim/TargetSet.cs` | the places one bus is aimed at, and how many warheads each gets — **data only**, so a set of one target is exactly the salvo that flies today. Spare warheads stay **aboard** rather than being spread, and warheads sharing a target are released **together**: a bus's own rounds are outside its own blast sweep, so a stagger buys nothing and costs 8.33 m/s of divert against 2.91 for 2 km of ground |
| `Sim/DivertFootprint.cs` | how far one bus can move its own landing, as an ellipse on the ground — **read off the columns `ReleaseFocus` already flies for the kicks**, so the display costs no flying, and the long axis is `450 · cot γ` rather than a typed shape. It is the **free-clock** reach: the flight pins the arrival, which costs 2.56x along the track at 6,179 km and nothing across it |
| `Sim/ReleaseItinerary.cs` | when a bus with several targets lets each one's warheads go — **started early enough that the last release still lands on today's gate**, because the reach from that gate is 3.5 km falling to 0.4 against a 2.0 km lethal radius, and because a set of one is then exactly the shot everything else is measured against. Farthest reach first, and it **says what does not fit** rather than truncating: six targets need 66.1 m/s against a 60 m/s cap |
